...
for table_name in ${!table_columns[*]}; do
    ...
    # 要批量执行的命令放在大括号内, 后台运行
    {
        for file in "${TPCDS_DATA_DIR}/${table_name}"_{1..100}_*.dat; do
            ...
            echo "Loading ${file} to https://${FE_HOST}:${FE_HTTPS_PORT:-29991}/api/${DB}/${table_name}/_stream_load"
            if [[ -z ${TXN_ID} ]]; then
                ret=$(curl -k -s --location-trusted \
                    -u "${USER}":"${PASSWORD:-}" \
                    -H "Expect: 100-continue" \
                    -H "column_separator:|" \
                    -H "columns: ${table_columns[${table_name}]}" \
                    -T "${file}" \
                    https://"${FE_HOST}":"${FE_HTTPS_PORT:-29991}"/api/"${DB}"/"${table_name}"/_stream_load)
            else
                ret=$(curl -k -s --location-trusted \
                    -u "${USER}":"${PASSWORD:-}" \
                    -H "Expect: 100-continue" \
            ...
            echo "Response: ${ret}"
            if echo "${ret}" | grep -q '"Status"\s*:\s*"Success"'; then
                echo "----loaded ${file}"
            else
