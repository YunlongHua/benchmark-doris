...
if [[ ${SCALE_FACTOR} -eq 1 ]]; then
    ...
    export MYSQL_PWD="${PASSWORD}"
    LIBMYSQL_ENABLE_CLEARTEXT_PLUGIN=1 mysql -h"${FE_HOST}" -u"${USER}" -P"${FE_QUERY_PORT}" -D"${DB}" <"${CURDIR}"/../ddl/create-ssb-tables-sf1.sql
    unset MYSQL_PWD

    echo "Run SQLs from ${CURDIR}/../ddl/create-ssb-flat-tables-sf1.sql"
    export MYSQL_PWD="${PASSWORD}"
    LIBMYSQL_ENABLE_CLEARTEXT_PLUGIN=1 mysql -h"${FE_HOST}" -u"${USER}" -P"${FE_QUERY_PORT}" -D"${DB}" <"${CURDIR}"/../ddl/create-ssb-flat-tables-sf1.sql
    unset MYSQL_PWD
elif [[ ${SCALE_FACTOR} -eq 100 ]]; then
    echo "Run SQLs from ${CURDIR}/../ddl/create-ssb-tables-sf100.sql"
    export MYSQL_PWD="${PASSWORD}"
    LIBMYSQL_ENABLE_CLEARTEXT_PLUGIN=1 mysql -h"${FE_HOST}" -u"${USER}" -P"${FE_QUERY_PORT}" -D"${DB}" <"${CURDIR}"/../ddl/create-ssb-tables-sf100.sql
    unset MYSQL_PWD

    echo "Run SQLs from ${CURDIR}/../ddl/create-ssb-flat-tables-sf100.sql"
    export MYSQL_PWD="${PASSWORD}"
    LIBMYSQL_ENABLE_CLEARTEXT_PLUGIN=1 mysql -h"${FE_HOST}" -u"${USER}" -P"${FE_QUERY_PORT}" -D"${DB}" <"${CURDIR}"/../ddl/create-ssb-flat-tables-sf100.sql
    unset MYSQL_PWD
elif [[ ${SCALE_FACTOR} -eq 1000 ]]; then
    echo "Run SQLs from ${CURDIR}/../ddl/create-ssb-tables-sf1000.sql"
    export MYSQL_PWD="${PASSWORD}"
    LIBMYSQL_ENABLE_CLEARTEXT_PLUGIN=1 mysql -h"${FE_HOST}" -u"${USER}" -P"${FE_QUERY_PORT}" -D"${DB}" <"${CURDIR}"/../ddl/create-ssb-tables-sf1000.sql
    unset MYSQL_PWD

    echo "Run SQLs from ${CURDIR}/../ddl/create-ssb-flat-tables-sf1000.sql"
    export MYSQL_PWD="${PASSWORD}"
    LIBMYSQL_ENABLE_CLEARTEXT_PLUGIN=1 mysql -h"${FE_HOST}" -u"${USER}" -P"${FE_QUERY_PORT}" -D"${DB}" <"${CURDIR}"/../ddl/create-ssb-flat-tables-sf1000.sql
    unset MYSQL_PWD
else
