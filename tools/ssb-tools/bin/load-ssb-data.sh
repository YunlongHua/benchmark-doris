...
if [[ -z ${TXN_ID} ]]; then
    curl -k --location-trusted -u "${USER}":"${PASSWORD}" \
        ...
        -T "${SSB_DATA_DIR}"/part.tbl https://"${FE_HOST}":"${FE_HTTPS_PORT}"/api/"${DB}"/part/_stream_load
else
    curl -k --location-trusted -u "${USER}":"${PASSWORD}" \
...
if [[ -z ${TXN_ID} ]]; then
    curl -k --location-trusted -u "${USER}":"${PASSWORD}" \
        ...
        -T "${SSB_DATA_DIR}"/date.tbl https://"${FE_HOST}":"${FE_HTTPS_PORT}"/api/"${DB}"/dates/_stream_load
else
    curl -k --location-trusted -u "${USER}":"${PASSWORD}" \
...
if [[ -z ${TXN_ID} ]]; then
    curl -k --location-trusted -u "${USER}":"${PASSWORD}" \
        ...
        -T "${SSB_DATA_DIR}"/supplier.tbl https://"${FE_HOST}":"${FE_HTTPS_PORT}"/api/"${DB}"/supplier/_stream_load
else
    curl -k --location-trusted -u "${USER}":"${PASSWORD}" \
...
if [[ -z ${TXN_ID} ]]; then
    curl -k --location-trusted -u "${USER}":"${PASSWORD}" \
        ...
        -T "${SSB_DATA_DIR}"/customer.tbl https://"${FE_HOST}":"${FE_HTTPS_PORT}"/api/"${DB}"/customer/_stream_load
else
    curl -k --location-trusted -u "${USER}":"${PASSWORD}" \
...
function load() {
    ...
    if [[ -z ${TXN_ID} ]]; then
        curl -k --location-trusted -u "${USER}":"${PASSWORD}" \
            ...
            -T "$@" https://"${FE_HOST}":"${FE_HTTPS_PORT}"/api/"${DB}"/lineorder/_stream_load
    else
        curl -k --location-trusted -u "${USER}":"${PASSWORD}" \
