...
if [[ -d ${TPCH_DATA_DIR}/ ]]; then
    echo "Removing existing ${TPCH_DATA_DIR}..."
    rm -rf "${TPCH_DATA_DIR}"
fi
...
for i in $(seq 1 "${PARALLEL}"); do
    {
        "${TPCH_DBGEN_DIR}"/dbgen -f -s "${SCALE_FACTOR}" -T S -C "${PARALLEL}" -S "${i}" >/dev/null 2>&1
    } &
done
wait
...
for i in $(seq 1 "${PARALLEL}"); do
    {
        "${TPCH_DBGEN_DIR}"/dbgen -f -s "${SCALE_FACTOR}" -T O -C "${PARALLEL}" -S "${i}" >/dev/null 2>&1
    } &
done
wait
...
for i in $(seq 1 "${PARALLEL}"); do
    {
        "${TPCH_DBGEN_DIR}"/dbgen -f -s "${SCALE_FACTOR}" -T L -C "${PARALLEL}" -S "${i}" >/dev/null 2>&1
    } &
done
wait
