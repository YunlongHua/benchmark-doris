...
_has_fe_http_port=0; env | grep -q '^FE_HTTP_PORT=' && _has_fe_http_port=1
_has_fe_https_port=0; env | grep -q '^FE_HTTPS_PORT=' && _has_fe_https_port=1
...
[[ ${_has_fe_http_port} -eq 1 ]] && FE_HTTP_PORT="${FE_HTTP_PORT}"
[[ ${_has_fe_https_port} -eq 1 ]] && FE_HTTPS_PORT="${FE_HTTPS_PORT}"
...
echo "FE_HTTPS_PORT: ${FE_HTTPS_PORT:-29991}"
echo "USER: ${USER}"
echo "DB: ${DB}"
...
function load_region() {
    echo "$*"
    if [[ -z ${TXN_ID} ]]; then
        curl -sk --location-trusted -u "${USER}":"${PASSWORD}" -H "Expect: 100-continue" -H "column_separator:|" \
            -H "columns: r_regionkey, r_name, r_comment, temp" \
            -T "$*" https://"${FE_HOST}":"${FE_HTTPS_PORT:-29991}"/api/"${DB}"/region/_stream_load
    else
        curl -sk --location-trusted -u "${USER}":"${PASSWORD}" -H "Expect: 100-continue" -H "column_separator:|" \
...
function load_nation() {
    ...
    if [[ -z ${TXN_ID} ]]; then
        curl -sk --location-trusted -u "${USER}":"${PASSWORD}" -H "Expect: 100-continue" -H "column_separator:|" \
            ...
            -T "$*" https://"${FE_HOST}":"${FE_HTTPS_PORT:-29991}"/api/"${DB}"/nation/_stream_load
    else
        curl -sk --location-trusted -u "${USER}":"${PASSWORD}" -H "Expect: 100-continue" -H "column_separator:|" \
...
function load_supplier() {
    ...
    if [[ -z ${TXN_ID} ]]; then
        curl -sk --location-trusted -u "${USER}":"${PASSWORD}" -H "Expect: 100-continue" -H "column_separator:|" \
            ...
            -T "$*" https://"${FE_HOST}":"${FE_HTTPS_PORT:-29991}"/api/"${DB}"/supplier/_stream_load
    else
        curl -sk --location-trusted -u "${USER}":"${PASSWORD}" -H "Expect: 100-continue" -H "column_separator:|" \
...
function load_customer() {
    ...
    if [[ -z ${TXN_ID} ]]; then
        curl -sk --location-trusted -u "${USER}":"${PASSWORD}" -H "Expect: 100-continue" -H "column_separator:|" \
            ...
            -T "$*" https://"${FE_HOST}":"${FE_HTTPS_PORT:-29991}"/api/"${DB}"/customer/_stream_load
    else
        curl -sk --location-trusted -u "${USER}":"${PASSWORD}" -H "Expect: 100-continue" -H "column_separator:|" \
...
function load_part() {
    ...
    if [[ -z ${TXN_ID} ]]; then
        curl -sk --location-trusted -u "${USER}":"${PASSWORD}" -H "Expect: 100-continue" -H "column_separator:|" \
            ...
            -T "$*" https://"${FE_HOST}":"${FE_HTTPS_PORT:-29991}"/api/"${DB}"/part/_stream_load
    else
        curl -sk --location-trusted -u "${USER}":"${PASSWORD}" -H "Expect: 100-continue" -H "column_separator:|" \
...
function load_partsupp() {
    ...
    if [[ -z ${TXN_ID} ]]; then
        curl -sk --location-trusted -u "${USER}":"${PASSWORD}" -H "Expect: 100-continue" -H "column_separator:|" \
            ...
            -T "$*" https://"${FE_HOST}":"${FE_HTTPS_PORT:-29991}"/api/"${DB}"/partsupp/_stream_load
    else
        curl -sk --location-trusted -u "${USER}":"${PASSWORD}" -H "Expect: 100-continue" -H "column_separator:|" \
...
function load_orders() {
    ...
    if [[ -z ${TXN_ID} ]]; then
        curl -sk --location-trusted -u "${USER}":"${PASSWORD}" -H "Expect: 100-continue" -H "column_separator:|" \
            ...
            -T "$*" https://"${FE_HOST}":"${FE_HTTPS_PORT:-29991}"/api/"${DB}"/orders/_stream_load
    else
        curl -sk --location-trusted -u "${USER}":"${PASSWORD}" -H "Expect: 100-continue" -H "column_separator:|" \
...
function load_lineitem() {
    ...
    if [[ -z ${TXN_ID} ]]; then
        curl -sk --location-trusted -u "${USER}":"${PASSWORD}" -H "Expect: 100-continue" -H "column_separator:|" \
            ...
            -T "$*" https://"${FE_HOST}":"${FE_HTTPS_PORT:-29991}"/api/"${DB}"/lineitem/_stream_load
    else
        curl -sk --location-trusted -u "${USER}":"${PASSWORD}" -H "Expect: 100-continue" -H "column_separator:|" \
...
# 以PID为名, 防止创建命名管道时与已有文件重名，从而失�?
fifo="/tmp/$$.fifo"
# 创建命名管道
mkfifo "${fifo}"
# 以读写方式打开命名管道，文件标识符fd�?，fd可取�?�?�?�?�?-9中的任意数字
exec 3<>"${fifo}"
# 删除文件, 也可不删�? 不影响后面操�?
rm -rf "${fifo}"

# 在fd3中放�?PARALLEL个空行作为令�?
...
run_sql() {
    echo "$*"
    LIBMYSQL_ENABLE_CLEARTEXT_PLUGIN=1 MYSQL_PWD="${PASSWORD}" mysql -h"${FE_HOST}" -u"${USER}" -P"${FE_QUERY_PORT}" -D"${DB}" -e "$*"
}
start=$(date +%s)
